...
## Announcements
* Tuesday: Last wrap on bslib/dashboards
* 3/11 final due!!
...
* Build on last class
* More on layouts, including tabsets 
* Small discussion of themes
* Prep for dashboards
...
We've previously worked in a somewhat standard layout: panels on the side and main -- everything is on one page. 

* How to divide the page 
* How to 

---
## Fluidity: dynamic page adjustment

If you want to use the column grid system (and adjust width based on item size), you can use fluidRow. This is our prior example: 
...
```{r, echo = FALSE}
knitr::include_graphics("https://shiny.posit.co/r/articles/build/layout-guide/pages.png")
```
...
* Test out a different theme
* Add this plot under the prior plot and fix the color issue :) 

```{r, eval = FALSE}
  output$myplot2 <-renderPlot({
    ggplot(select_data(), aes(x = fct_infreq(eye_color))) + geom_bar(col = "orchid") +
      labs(
         title  = "Bar plot of eye color", 
         x = "Eyecolor")
  })
```
...
knitr::include_url("https://shiny.posit.co/r/layouts/")
```
