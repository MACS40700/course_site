...
```{r, message = FALSE}
# NEED ALL ADDITIONAL FILES TOO!
#regional shapefile:
sf_regional <- st_read("ComunidadesAutonomas_ETRS89_30N/Comunidades_Autonomas_ETRS89_30N.shp") 


#Converting it to df:
regional_df <- sf_to_df(sf_regional, fill = TRUE) %>% mutate(id = as.numeric("Codigo"))
```

There's a slight problem here, which is that by doing `tidy()`, we're losing all information identifying our regional units (the "id" column doesn't really tell us which region corresponds to each id). This can be easily fixed by creating a temporary data frame with the name/code of the regions (which is in the sahapefile) and then joining it to our `regional_df` df.
...
temp_df$id <- as.numeric(as.character(seq(0,nrow(temp_df)-1)))
...
```{r}
#local shapefile:
sf_local <- st_read("Municipios_IGN-shp/Municipios_IGN.shp")
#Converting it to df:
local_df <- sf_to_df(sf_local) %>% mutate(id = as.numeric("FID"))

# Recover row name
temp_df2 <- data.frame(sf_local$CODIGOINE)
# Create and append id
temp_df2$id <- as.numeric(as.character(seq(0,nrow(temp_df2)-1)))
#joining
...
It's finally time to plot! We'll start with a very basic plot before getting picky with the details:
...
#you should be using the following aesthetics for any plot you make:
  ...
  theme_minimal()+ ggsave("uglyplot.png", height = 5, width = 7)
```
...
If we want to have the Canary Islands closer in the map, we'll need to bring them closer. I asked on Twitter a couple of days ago whether anyone had a nice, not painful idea on how to achieve this, but most people told me they just normally do two maps and then copy-paste them.
...
Last but not least, we'll be applying all the logic from above to create a map at the local level. I'll be using data on **average income at the local level**  (variable: `renta`) that can be downloaded from [here](https://www.ine.es/experimental/atlas/exp_atlas_tab.htm). The steps to be followed are the same:
...
local_plot <- local_df2%>%left_join(income_data, by = "ine_cod")%>%
...
#Df for the Canary Islands separating line 
#(things change a bit in this shapefile)
