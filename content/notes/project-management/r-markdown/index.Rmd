...
Until now, you have only run code in a specially designated chunk. However you can also run R code **in-line** by using the `` `r knitr::inline_expr("")` `` syntax. For example, look at the text from the example document earlier:
...
```{r yml-code-fold, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(html_document(
    code_folding = "hide"
  ))
...
```{r yml-keep-md, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(html_document(
    keep_md = TRUE
  ))
...
```{r yml-pdf-toc, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(pdf_document(
    toc = TRUE,
    toc_depth = TRUE
...
```{r yml-pdf-highlight, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(pdf_document(
    highlight = "pygments"
  ))
...
```{r yml-pdf-latex-opts, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(pdf_document()) %>%
  yml_latex_opts(fontsize = "11pt", geometry = "margin=1in")
```
...
```{r yml-pdf-keep-tex, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(pdf_document(
    keep_tex = TRUE
  ))
...
```{r yml-multiple, comment = "", dependson = "yml-default"}
yml_default %>%
  yml_output(
    pdf_document(),
    html_document(
